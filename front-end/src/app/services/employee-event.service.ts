...
export class EmployeeEventService {
  ...
  getEntranceExitEvents(employeeId: string, selectedDay: string): Observable<EmployeeEvent[]> {
    return this.http.get<EmployeeEvent[]>(`${this.baseUrl}${employeeId}/events/entrance-exit/${selectedDay}`);
  }
}
